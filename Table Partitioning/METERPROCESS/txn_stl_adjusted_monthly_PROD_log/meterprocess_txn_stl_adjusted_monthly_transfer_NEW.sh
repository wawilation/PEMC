...
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "set session_replication_role = replica;" -c "DELETE FROM meterprocess.txn_stl_adjusted_monthly_c3mos A WHERE A.reading_datetime < '$FINAL_DATE_c3mos';"	
	...
	if [ "$?" = "0" ]; then 
	#vacuum analyze
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "VACUUM ANALYZE meterprocess.txn_stl_adjusted_monthly_c3mos;" -c "VACUUM ANALYZE meterprocess.txn_stl_adjusted_monthly_h13mos;"		
...
SELECT * FROM ONLY meterprocess.txn_stl_adjusted_monthly_h13mos WHERE created_datetime < '$FINAL_DATE_h13mos') TO '/tmp/crss_meterprocess.txn_stl_adjusted_monthly_h13mos_$FINAL_DATE_h13mos.dat';"
gzip /tmp/crss_meterprocess.txn_stl_adjusted_monthly_h13mos_$FINAL_DATE_h13mos.dat					

					if [ "$?" = "0" ]; then
					#delete from historical 13 mos
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "set session_replication_role = replica;" -c "DELETE FROM meterprocess.txn_stl_adjusted_monthly_h13mos A WHERE A.created_datetime < '$FINAL_DATE_h13mos';"

						if [ "$?" = "0" ]; then 
						#vacuum analyze
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "VACUUM ANALYZE meterprocess.txn_stl_adjusted_monthly_h13mos;"						
