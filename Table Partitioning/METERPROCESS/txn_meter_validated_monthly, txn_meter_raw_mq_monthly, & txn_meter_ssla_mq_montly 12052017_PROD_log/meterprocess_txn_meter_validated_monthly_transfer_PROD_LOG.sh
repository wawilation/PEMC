...
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "INSERT INTO meterprocess.partition_log SELECT '$ID','meterprocess.txn_meter_validated_monthly_c3mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, A.count, NULL, 'DC'
...
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "VACUUM ANALYZE meterprocess.txn_meter_ssla_mq_monthly_c3mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_raw_mq_monthly_c3mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_validated_monthly_c3mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_validated_monthly_h13mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_raw_mq_monthly_h13mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_ssla_mq_monthly_h13mos;"		
		...
		if [ "$?" = "0" ]; then
		#####LOG HERE (status = 'V')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "INSERT INTO meterprocess.partition_log SELECT '$ID','(meterprocess) txn_meter_validated_monthly_c3mos,txn_meter_validated_monthly_h13mos,txn_meter_raw_mq_monthly_c3mos,txn_meter_raw_mq_monthly_h13mos,txn_meter_ssla_mq_monthly_c3mos,txn_meter_ssla_mq_monthly_h13mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, 0,NULL,'V';"
...
SELECT * FROM ONLY meterprocess.txn_meter_validated_monthly_h13mos WHERE created_date_time < '$FINAL_DATE_h13mos') TO '/pg_backup/data_archive/crss_meterprocess.txn_meter_validated_monthly_h13mos_$FINAL_DATE_h13mos.dat';"
gzip -f /pg_backup/data_archive/crss_meterprocess.txn_meter_validated_monthly_h13mos_$FINAL_DATE_h13mos.dat					

			if [ "$?" = "0" ]; then
			#####LOG HERE (status = 'A', count of recs archived based on status = 'XX')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "UPDATE meterprocess.partition_log SET status = 'A', controldate = current_timestamp WHERE id = '$ID' AND object = 'meterprocess.txn_meter_validated_monthly_h13mos' AND status = 'XX' AND archivepath IS NOT NULL;"
...
gzip -f /pg_backup/data_archive/crss_meterprocess.txn_meter_raw_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat				

				if [ "$?" = "0" ]; then
				#####LOG HERE (status = 'A', count of recs archived based on status = 'XX')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "UPDATE meterprocess.partition_log SET status = 'A', controldate = current_timestamp WHERE id = '$ID' AND object = 'meterprocess.txn_meter_raw_mq_monthly_h13mos' AND status = 'XX' AND archivepath IS NOT NULL;"
...
gzip -f /pg_backup/data_archive/crss_meterprocess.txn_meter_ssla_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat					

					if [ "$?" = "0" ]; then
					#####LOG HERE (status = 'A', count of recs archived based on status = 'XX')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "UPDATE meterprocess.partition_log SET status = 'A', controldate = current_timestamp WHERE id = '$ID' AND object = 'meterprocess.txn_meter_ssla_mq_monthly_h13mos' AND status = 'XX' AND archivepath IS NOT NULL;"
...
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "INSERT INTO meterprocess.partition_log SELECT '$ID','meterprocess.txn_meter_validated_monthly_h13mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, A.count, A.archivepath, 'DH'
FROM meterprocess.partition_log A WHERE A.id = '$ID' AND A.object = 'meterprocess.txn_meter_validated_monthly_h13mos' AND A.status = 'A';" -c "INSERT INTO meterprocess.partition_log SELECT '$ID','meterprocess.txn_meter_raw_mq_monthly_h13mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, A.count, A.archivepath, 'DH' FROM meterprocess.partition_log A WHERE A.id = '$ID' AND A.object = 'meterprocess.txn_meter_raw_mq_monthly_h13mos' AND A.status = 'A';" -c "INSERT INTO meterprocess.partition_log SELECT '$ID','meterprocess.txn_meter_ssla_mq_monthly_h13mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, A.count, A.archivepath, 'DH' FROM meterprocess.partition_log A WHERE A.id = '$ID' AND A.object = 'meterprocess.txn_meter_ssla_mq_monthly_h13mos' AND A.status = 'A';"
						...
						#vacuum analyze
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "VACUUM ANALYZE meterprocess.txn_meter_validated_monthly_h13mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_raw_mq_monthly_h13mos;" -c "VACUUM ANALYZE meterprocess.txn_meter_ssla_mq_monthly_h13mos;"


						if [ "$?" = "0" ]; then
						#####LOG HERE (status = 'V')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "INSERT INTO meterprocess.partition_log SELECT '$ID','(meterprocess) txn_meter_validated_monthly_h13mos,txn_meter_raw_mq_monthly_h13mos,txn_meter_ssla_mq_monthly_h13mos', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, 0,NULL,'V';"
...
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "ALTER TABLE ONLY meterprocess.txn_meter_validated_monthly_c3mos DROP CONSTRAINT IF EXISTS txn_meter_validated_monthly_c3mos_ck; ALTER TABLE ONLY meterprocess.txn_meter_validated_monthly_c3mos ADD CONSTRAINT txn_meter_validated_monthly_c3mos_ck CHECK (reading_datetime >= '$FINAL_DATE_c3mos'); ALTER TABLE ONLY meterprocess.txn_meter_validated_monthly_h13mos DROP CONSTRAINT IF EXISTS txn_meter_validated_monthly_h13mos_ck; ALTER TABLE ONLY meterprocess.txn_meter_validated_monthly_h13mos ADD CONSTRAINT txn_meter_validated_monthly_h13mos_ck CHECK (reading_datetime < '$FINAL_DATE_c3mos');"
							...
							if [ "$?" = "0" ]; then
							#####LOG HERE (status = 'C')
/Postgres/edb/as9.6/bin/psql -p 5432 -U postgres -d crss -c "INSERT INTO meterprocess.partition_log SELECT '$ID','(meterprocess) txn_meter_validated_monthly_c3mos_ck CHECK (reading_datetime >= ''$FINAL_DATE_c3mos''),txn_meter_validated_monthly_h13mos_ck CHECK (reading_datetime < ''$FINAL_DATE_c3mos'')', '$FINAL_DATE_c3mos', '$FINAL_DATE_h13mos', current_timestamp, 0,NULL,'C';"
