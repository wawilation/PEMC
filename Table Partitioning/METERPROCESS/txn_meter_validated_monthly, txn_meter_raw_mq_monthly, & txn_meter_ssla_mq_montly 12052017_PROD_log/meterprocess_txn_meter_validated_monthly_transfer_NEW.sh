...
SELECT * FROM ONLY meterprocess.txn_meter_validated_monthly_h13mos WHERE created_date_time < '$FINAL_DATE_h13mos') TO '/tmp/crss_meterprocess.txn_meter_validated_monthly_h13mos_$FINAL_DATE_h13mos.dat';"
gzip /tmp/crss_meterprocess.txn_meter_validated_monthly_h13mos_$FINAL_DATE_h13mos.dat					

			if [ "$?" = "0" ]; then
			#dump older than historical13mos data to archive file	
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "COPY(
SELECT B.* FROM meterprocess.txn_meter_validated_monthly_h13mos A JOIN meterprocess.txn_meter_raw_mq_monthly_h13mos B ON A.id = B.validated_id WHERE A.created_date_time < '$FINAL_DATE_h13mos') TO '/tmp/crss_meterprocess.txn_meter_raw_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat';"
gzip /tmp/crss_meterprocess.txn_meter_raw_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat				

				if [ "$?" = "0" ]; then
				#dump older than historical13mos data to archive file
/usr/pgsql-9.6/bin/psql -p 5432 -U postgres -d crss_table_partitioning -c "COPY(
SELECT C.* FROM meterprocess.txn_meter_validated_monthly_h13mos A JOIN meterprocess.txn_meter_raw_mq_monthly_h13mos B ON A.id = B.validated_id JOIN meterprocess.txn_meter_ssla_mq_monthly_h13mos C ON B.id = C.monthly_mq_id WHERE A.created_date_time < '$FINAL_DATE_h13mos') TO '/tmp/crss_meterprocess.txn_meter_ssla_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat';"
gzip /tmp/crss_meterprocess.txn_meter_ssla_mq_monthly_h13mos_$FINAL_DATE_h13mos.dat					
